...
func UserUpdateRole(ctx context.Context, db *gorm.DB, id uint, role string) error {
	result := dbWithContext(ctx, db).
		Model(&libdb.User{}).
		Where("id = ?", id).
		Update("role", role)
...
func UserCountByRole(ctx context.Context, db *gorm.DB, role string) (int64, error) {
	var count int64
	if err := dbWithContext(ctx, db).
		Model(&libdb.User{}).
		Where("role = ?", role).
